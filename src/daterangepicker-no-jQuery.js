...
* @param {HTMLElement} element - The parent element that the date range picker will be added to
...
class DateRangePicker {
    constructor(element, options, cb) {
        ...
        * @typedef Options 
        * @property {string} parentEl=body - {@link https://developer.mozilla.org/en-US/docs/Web/CSS/Guides/Selectors|CSS selector} of the parent element that the date range picker will be added to
        ...
        * @property {string|Array} altInput=null - A {@link https://developer.mozilla.org/en-US/docs/Web/CSS/Guides/Selectors|CSS selector} string for an alternative
        ...
        */
        ...
        on(drpCalendar, 'click.daterangepicker', '.prev', this.clickPrev.bind(this));
        on(drpCalendar, 'click.daterangepicker', '.next', this.clickNext.bind(this));
        on(drpCalendar, 'mousedown.daterangepicker', 'td.available', this.clickDate.bind(this));
        on(drpCalendar, 'mouseenter.daterangepicker', 'td.available', this.hoverDate.bind(this));
        on(drpCalendar, 'change.daterangepicker', 'select.yearselect', this.monthOrYearChanged.bind(this));
        on(drpCalendar, 'change.daterangepicker', 'select.monthselect', this.monthOrYearChanged.bind(this));
        on(drpCalendar, 'change.daterangepicker', 'select.hourselect,select.minuteselect,select.secondselect,select.ampmselect', this.timeChanged.bind(this));

        const drpRanges = this.container.querySelector('.ranges');
        on(drpRanges, 'click.daterangepicker', 'li', this.clickRange.bind(this));
        on(drpRanges, 'mouseenter.daterangepicker', 'li', this.hoverRange.bind(this));
        on(drpRanges, 'mouseleave.daterangepicker', 'li', this.leaveRange.bind(this));

        const drpButtons = this.container.querySelector('.drp-buttons');
        on(drpButtons, 'click.daterangepicker', 'button.applyBtn', this.clickApply.bind(this));
        on(drpButtons, 'click.daterangepicker', 'button.cancelBtn', this.clickCancel.bind(this));

        on(this.element, 'click.daterangepicker', this.toggle.bind(this));
        on(this.element, 'keydown.daterangepicker', this.toggle.bind(this));//IE 11 compatibility
        if (this.element.matches('input') || this.element.matches('button')) {
            on(this.element, 'focus.daterangepicker', this.show.bind(this));
            on(this.element, 'keyup.daterangepicker', this.elementChanged.bind(this));
        }
    ...
    * const drp = document.querySelector('#picker')._daterangepicker;
    ...
    */
    setStartDate(startDate, isValid = false) {
        ...
        if (!this.singleDatePicker && !this.endDate) {
            ...
            this.container.querySelector('.drp-selected').innerHTML = this.formatDate(this.startDate) + this.locale.separator + empty;
        }
    ...
    * Use this option only if you are sure about the value you put in.
    * @throws `RangeError` for invalid date values.
    * @example const drp = document.querySelector('#picker')._daterangepicker;
    ...
    setEndDate(endDate, isValid = false) {
        ...
        if (isValid === undefined || !isValid) {
            if (typeof endDate === 'object') {
                if (DateTime.isDateTime(endDate) && endDate.isValid) {
        ...
        if (!this.singleDatePicker) {
            ...
            this.container.querySelector('.drp-selected').innerHTML = this.formatDate(this.startDate) + this.locale.separator + this.formatDate(this.endDate);
        }
    ...
    */
    setPeriod(startDate, endDate, isValid = false) {
        if (this.singleDatePicker) {
    ...
    * Correction can be skipped by `preventDefault` or returning `true` at event listener for `violated` 
    ...
    */
    validateInput(range) {
        ...
        * @example 
        * // correct values, if you prefer jQuery
        * $('#picker').on('violated.daterangepicker', (ev, picker, result) => {
        *    result.startDate = correctedStart;
        *    result.endDate = correctedEnd;
        * })
        * // otherwise
        * document.querySelector('#picker').addEventListener('violated', e => {
        *    e.detail.startDate = correctedStart;
        *    e.detail.endDate = correctedEnd;
        * });
        * // keep wrong value
        * $('#picker').on('violated.daterangepicker', (ev, picker, result) => { return true })
        * document.querySelector('#picker').addEventListener('violated', e => { e.preventDefault() });
        */
        ...
        if (this.singleDatePicker) {
            ...
            if (range === undefined) {
                if (result.startDate.violations.length > 0) {
                    const payload = { picker: this, startDate, endDate, violations: result };
                    this.element.dispatchEvent(new CustomEvent('violated', { detail: payload, bubbles: false, cancelable: true }));
    ...
    */
    updateView() {
        if (this.timePicker) {
            this.element.dispatchEvent(new CustomEvent('beforeRenderTimePicker', { detail: this, bubbles: true }));
            ...
            if (!this.endDate) {
                this.container.querySelector('.calendar-time.end-time select').classList.add('disabled');
            } else {
                this.container.querySelector('.calendar-time.end-time select').classList.remove('disabled');
            }
        }
        if (this.endDate) {
    ...
    */
    updateCalendars() {
        ...
        * @param {DateRangePicker} this - The daterangepicker object
        */
        this.element.dispatchEvent(new CustomEvent('beforeRenderCalendar', { detail: this, bubbles: true }));
        this.renderCalendar('left');
        //if (!this.singleMonthView)
        this.renderCalendar('right');

        //highlight any predefined range matching the current start and end dates
        this.container.querySelector('.ranges li')?.classList.remove('active');
    ...
    */
    updateFormInputs() {
        const enabled = (this.singleDatePicker || (this.endDate && this.startDate <= this.endDate));
        this.container.querySelector('button.applyBtn').disabled = !enabled;
    }
    ...
    */
    move() {
        ...
        var containerWidth = outerWidth(this.container);
    ...
    */
    show(e) {
        ...
        on(document, 'mousedown.daterangepicker', this._outsideClickProxy);
        // also support mobile devices
        on(document, 'touchend.daterangepicker', this._outsideClickProxy);
        // also explicitly play nice with Bootstrap dropdowns, which stopPropagation when clicking them
        on(document, 'click.daterangepicker', '[data-toggle=dropdown]', this._outsideClickProxy);
        // and also close when focus changes to outside the picker (eg. tabbing between controls)
        on(document, 'focusin.daterangepicker', this._outsideClickProxy);

        // Reposition the picker if the window is resized while it's open
        on(window, 'resize.daterangepicker', function (e) { this.move(e); }.bind(this));
        ...
        * @param {DateRangePicker} this - The daterangepicker object
        */
        this.element.dispatchEvent(new CustomEvent('show', { detail: this, bubbles: true }));
    ...
    */
    hide(e) {
        ...
        * $('#el').on('beforeHide', function (ev, picker) {
        *   return true;
        * });
        * // Plain Vanilla-JS
        * pickerEl.addEventListener('beforeHide', e => {
        *   const picker = e.detail;
        *   e.preventDefault();
        * });
        */
        const event = new CustomEvent('beforeHide', { detail: this, bubbles: false, cancelable: true })
        this.element.dispatchEvent(event);
        if (event.defaultPrevented)
            return;

        offNamespace(document, '.daterangepicker');
        offNamespace(window, '.daterangepicker');
        ...
        * @param {DateRangePicker} this - The daterangepicker object
        */
        this.element.dispatchEvent(new CustomEvent('hide', { detail: this, bubbles: true }));
    ...
    */
    outsideClick(e) {
        ...
        // itself then call this.style.display = 'none'
        if (
            // ie modal dialog fix
            e.type === "focusin" ||
        ...
        * @param {DateRangePicker} this - The daterangepicker object
        */
        this.element.dispatchEvent(new CustomEvent('outsideClick', { detail: this, bubbles: true }));
    ...
    */
    showCalendars() {
        ...
        * @param {DateRangePicker} this - The daterangepicker object
        */
        this.element.dispatchEvent(new CustomEvent('showCalendar', { detail: this, bubbles: true }));
    ...
    */
    hideCalendars() {
        ...
        * @param {DateRangePicker} this - The daterangepicker object
        */
        this.element.dispatchEvent(new CustomEvent('hideCalendar', { detail: this, bubbles: true }));
    ...
    */
    clickRange(e) {
        ...
        if (label == this.locale.customRangeLabel) {
            ...
            this.element.dispatchEvent(new CustomEvent('beforeHide', { detail: this, bubbles: false, cancelable: true }));
            if (e.defaultPrevented)
                this.updateView();
    ...
    */
    clickPrev(e) {
        var cal = ancestors(e.target, '.drp-calendar');
    ...
    */
    clickDate(e) {
        ...
        */
        this.element.dispatchEvent(new CustomEvent('hideCalendar', { detail: [this, side], bubbles: true }));
    ...
    calculateChosenLabel() {
        ...
        for (var range in this.ranges) {
            ...
            if (this.startDate.startOf(unit).equals(this.ranges[range][0].startOf(unit)) && this.endDate.startOf(unit).equals(this.ranges[range][1].startOf(unit))) {
                customRange = false;
                this.chosenLabel = this.container.querySelector('.ranges li:eq(' + i + ')').classList.add('active').dataset.rangeKey;
                break;
            }
            i++;
        }
        if (customRange) {
            if (this.showCustomRangeLabel) {
                this.chosenLabel = this.container.querySelector('.ranges li:last-child')?.classList.add('active').dataset.rangeKey;
            } else {
    ...
    */
    clickApply(e) {
        ...
        * @param {DateRangePicker} this - The daterangepicker object
        */
        this.element.dispatchEvent(new CustomEvent('apply', { detail: this, bubbles: true }));
    }
    ...
    */
    clickCancel(e) {
        ...
        * @param {DateRangePicker} this - The daterangepicker object
        */
        this.element.dispatchEvent(new CustomEvent('cancel', { detail: this, bubbles: true }));
    }
    ...
    */
    timeChanged(e) {
        ...
        */
        this.element.dispatchEvent(new CustomEvent('timeChange', { detail: [this, this.singleDatePicker ? null : side], bubbles: true }));
    }
    ...
    */
    elementChanged(e) {
        ...
        if (this.singleDatePicker || start === null || end === null) {
            start = DateTime.fromFormat(this.element.value, format, { locale: DateTime.now().locale });
            end = start;
        }
        ...
        */
        if (trigger)
            this.element.dispatchEvent(new CustomEvent('inputChanged', { detail: this, bubbles: true }));
    ...
    updateElement() {
        ...
        if (this.altInput != null) {
            if (this.altFormat == null) {
                ...
                if (this.singleDatePicker) {
                    if (this.altInput.matches('input'))
                        this.altInput.value = this.startDate.toISO({ format: 'basic', precision: precision, includeOffset: false });
                } else {
                    if (this.altInput.every(x => x.matches('input'))) {
                        this.altInput[0].value = this.startDate.toISO({ format: 'basic', precision: precision, includeOffset: false });
                        if (this.endDate) {
                            this.altInput[1].value = this.endDate.toISO({ format: 'basic', precision: precision, includeOffset: false });
                        } else {
                ...
                if (this.singleDatePicker) {
                    if (this.altInput.matches('input'))
                        this.altInput.value = typeof this.altFormat === 'function' ? this.altFormat(this.startDate) : this.formatDate(this.startDate, this.altFormat);
                } else {
                    if (this.altInput.every(x => x.matches('input'))) {
                        this.altInput[0].value = typeof this.altFormat === 'function' ? this.altFormat(this.startDate) : this.formatDate(this.startDate, this.altFormat);
                        if (this.endDate) {
                            this.altInput[1].value = typeof this.altFormat === 'function' ? this.altFormat(this.endDate) : this.formatDate(this.endDate, this.altFormat);
                        } else {
                            this.altInput[1].value = null;
                        }
                    }
    ...
    */
    remove() {
        ...
        offNamespace(this.element, '.daterangepicker');
...
// implements jQuery on()
function on(el, eventName, selector, handler) {
...
function offNamespace(namespace) {
    for (let i = eventRegistry.length - 1; i >= 0; i--) {
        const entry = eventRegistry[i];
        if (entry.ns === namespace.replace('.', '')) {
            entry.el.removeEventListener(entry.evt, entry.listener);
...
* @function
* @param {string|HTMLElement|NodeList|HTMLElement} elements - Element where DateRangePicker is attached
* @param {Options} options - Object to configure the DateRangePicker
 ...
 * @returns The attached DateRangePicker
 */
