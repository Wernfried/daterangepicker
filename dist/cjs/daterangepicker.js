...
class DateRangePicker {
  constructor(element, options, cb) {
    this.parentEl = "body";
    this.element = (0, import_jquery.$)(element);
    this.startDate = import_luxon.DateTime.now().startOf("day");
    this.endDate = import_luxon.DateTime.now().endOf("day");
    this.minDate = null;
    this.maxDate = null;
    this.maxSpan = null;
    this.minSpan = null;
    this.defaultSpan = null;
    this.initalMonth = import_luxon.DateTime.now().startOf("month");
    this.autoApply = false;
    this.singleDatePicker = false;
    this.singleMonthView = false;
    this.showDropdowns = false;
    this.minYear = import_luxon.DateTime.now().minus({ year: 100 }).year;
    this.maxYear = import_luxon.DateTime.now().plus({ year: 100 }).year;
    this.showWeekNumbers = false;
    this.showISOWeekNumbers = false;
    this.showCustomRangeLabel = true;
    this.timePicker = false;
    const usesMeridiems = new Intl.DateTimeFormat(import_luxon.DateTime.now().locale, { hour: "numeric" }).resolvedOptions();
    this.timePicker24Hour = !usesMeridiems.hour12;
    this.timePickerStepSize = import_luxon.Duration.fromObject({ minutes: 1 });
    ...
    this.locale = {
      direction: "ltr",
      format: import_luxon.DateTime.DATE_SHORT,
      ...
      daysOfWeek: import_luxon.Info.weekdays("short"),
      monthNames: import_luxon.Info.months("long"),
      firstDay: import_luxon.Info.getStartOfWeek(),
      durationFormat: null
    };
    ...
    if (typeof options !== "object" || options === null)
      options = {};
    options = import_jquery.$.extend(this.element.data(), options);
    if (typeof options.externalStyle === "string" && ["bulma"].includes(options.externalStyle))
      this.externalStyle = options.externalStyle;
    if (typeof options.template !== "string" && !(options.template instanceof import_jquery.$)) {
      let template = [
    ...
    this.parentEl = options.parentEl && (0, import_jquery.$)(options.parentEl).length ? (0, import_jquery.$)(options.parentEl) : (0, import_jquery.$)(this.parentEl);
    this.container = (0, import_jquery.$)(options.template).appendTo(this.parentEl);
    if (typeof options.timePicker === "boolean")
      this.timePicker = options.timePicker;
    if (this.timePicker)
      this.locale.format = import_luxon.DateTime.DATETIME_SHORT;
    ...
    if (!this.singleDatePicker) {
      for (let opt of ["minSpan", "maxSpan", "defaultSpan"]) {
        if (["string", "number", "object"].includes(typeof options[opt])) {
          if (import_luxon.Duration.isDuration(options[opt]) && options[opt].isValid) {
            this[opt] = options[opt];
          } else if (import_luxon.Duration.fromISO(options[opt]).isValid) {
            this[opt] = import_luxon.Duration.fromISO(options[opt]);
          } else if (typeof options[opt] === "number" && import_luxon.Duration.fromObject({ seconds: options[opt] }).isValid) {
            this[opt] = import_luxon.Duration.fromObject({ seconds: options[opt] });
    ...
    if (this.timePicker) {
      if (typeof options.timePickerSeconds === "boolean")
        this.timePickerStepSize = import_luxon.Duration.fromObject({ [options.timePickerSeconds ? "seconds" : "minutes"]: 1 });
      if (typeof options.timePickerIncrement === "number")
        this.timePickerStepSize = import_luxon.Duration.fromObject({ minutes: options.timePickerIncrement });
      if (["string", "object", "number"].includes(typeof options.timePickerStepSize)) {
        let duration;
        if (import_luxon.Duration.isDuration(options.timePickerStepSize) && options.timePickerStepSize.isValid) {
          duration = options.timePickerStepSize;
        } else if (import_luxon.Duration.fromISO(options.timePickerStepSize).isValid) {
          duration = import_luxon.Duration.fromISO(options.timePickerStepSize);
        } else if (typeof options.timePickerStepSize === "number" && import_luxon.Duration.fromObject({ seconds: options.timePickerStepSize }).isValid) {
          duration = import_luxon.Duration.fromObject({ seconds: options.timePickerStepSize });
        } else {
        ...
        for (let unit of ["minutes", "seconds"])
          valid.push(...[1, 2, 3, 4, 5, 6, 10, 12, 15, 20, 30].map((x) => {
            return import_luxon.Duration.fromObject({ [unit]: x });
          }));
        valid.push(...[1, 2, 3, 4, 6].map((x) => {
          return import_luxon.Duration.fromObject({ hours: x });
        }));
      ...
      this.timePickerOpts = {
        showMinutes: this.timePickerStepSize < import_luxon.Duration.fromObject({ hours: 1 }),
        showSeconds: this.timePickerStepSize < import_luxon.Duration.fromObject({ minutes: 1 }),
        hourStep: this.timePickerStepSize >= import_luxon.Duration.fromObject({ hours: 1 }) ? this.timePickerStepSize.hours : 1,
        minuteStep: this.timePickerStepSize >= import_luxon.Duration.fromObject({ minutes: 1 }) ? this.timePickerStepSize.minutes : 1,
    ...
    for (let opt of ["startDate", "endDate", "minDate", "maxDate", "initalMonth"]) {
      ...
      if (typeof options[opt] === "object") {
        if (import_luxon.DateTime.isDateTime(options[opt]) && options[opt].isValid) {
          this[opt] = options[opt];
        } else if (options[opt] instanceof Date) {
          this[opt] = import_luxon.DateTime.fromJSDate(options[opt]);
        } else if (options[opt] === null) {
          this[opt] = null;
        } else {
        ...
        const format = typeof this.locale.format === "string" ? this.locale.format : import_luxon.DateTime.parseFormatForOpts(this.locale.format);
        if (import_luxon.DateTime.fromISO(options[opt]).isValid) {
          this[opt] = import_luxon.DateTime.fromISO(options[opt]);
        } else if (import_luxon.DateTime.fromFormat(options[opt], format, { locale: import_luxon.DateTime.now().locale }).isValid) {
          this[opt] = import_luxon.DateTime.fromFormat(options[opt], format, { locale: import_luxon.DateTime.now().locale });
        } else {
          const invalid = import_luxon.DateTime.fromFormat(options[opt], format, { locale: import_luxon.DateTime.now().locale }).invalidExplanation;
    ...
    if (typeof options.startDate === "undefined" && typeof options.endDate === "undefined") {
      if ((0, import_jquery.$)(this.element).is(":text")) {
        let start, end;
        const val = (0, import_jquery.$)(this.element).val();
        if (val != "") {
          const split = val.split(this.locale.separator);
          const format = typeof this.locale.format === "string" ? this.locale.format : import_luxon.DateTime.parseFormatForOpts(this.locale.format);
          if (split.length === 2) {
            start = import_luxon.DateTime.fromFormat(split[0], format, { locale: import_luxon.DateTime.now().locale });
            end = import_luxon.DateTime.fromFormat(split[1], format, { locale: import_luxon.DateTime.now().locale });
          } else if (this.singleDatePicker) {
            start = import_luxon.DateTime.fromFormat(val, format, { locale: import_luxon.DateTime.now().locale });
            end = import_luxon.DateTime.fromFormat(val, format, { locale: import_luxon.DateTime.now().locale });
          }
    ...
    if (!this.singleDatePicker && typeof options.ranges === "object") {
      for (let range in options.ranges) {
        ...
        if (["string", "object"].includes(typeof options.ranges[range][0])) {
          if (import_luxon.DateTime.isDateTime(options.ranges[range][0]) && options.ranges[range][0].isValid) {
            start = options.ranges[range][0];
          } else if (options.ranges[range][0] instanceof Date) {
            start = import_luxon.DateTime.fromJSDate(options.ranges[range][0]);
          } else if (typeof options.ranges[range][0] === "string" && import_luxon.DateTime.fromISO(options.ranges[range][0]).isValid) {
            start = import_luxon.DateTime.fromISO(options.ranges[range][0]);
          } else {
        ...
        if (["string", "object"].includes(typeof options.ranges[range][1])) {
          if (import_luxon.DateTime.isDateTime(options.ranges[range][1]) && options.ranges[range][1].isValid) {
            end = options.ranges[range][1];
          } else if (options.ranges[range][1] instanceof Date) {
            end = import_luxon.DateTime.fromJSDate(options.ranges[range][1]);
          } else if (typeof options.ranges[range][1] === "string" && import_luxon.DateTime.fromISO(options.ranges[range][1]).isValid) {
            end = import_luxon.DateTime.fromISO(options.ranges[range][1]);
          } else {
  ...
  setStartDate(startDate, isValid = false) {
    if (isValid === void 0 || !isValid) {
      if (typeof startDate === "object") {
        if (import_luxon.DateTime.isDateTime(startDate) && startDate.isValid) {
          this.startDate = startDate;
        } else if (startDate instanceof Date) {
          this.startDate = import_luxon.DateTime.fromJSDate(startDate);
        } else {
          throw RangeError(`The 'startDate' must be a luxon.DateTime or Date or string`);
        }
      } else if (typeof startDate === "string") {
        const format = typeof this.locale.format === "string" ? this.locale.format : import_luxon.DateTime.parseFormatForOpts(this.locale.format);
        if (import_luxon.DateTime.fromISO(startDate).isValid) {
          this.startDate = import_luxon.DateTime.fromISO(startDate);
        } else if (import_luxon.DateTime.fromFormat(startDate, format, { locale: import_luxon.DateTime.now().locale }).isValid) {
          this.startDate = import_luxon.DateTime.fromFormat(startDate, format, { locale: import_luxon.DateTime.now().locale });
        } else {
          const invalid = import_luxon.DateTime.fromFormat(startDate, format, { locale: import_luxon.DateTime.now().locale }).invalidExplanation;
  ...
  setEndDate(endDate2, isValid = false) {
    if (isValid === void 0 || !isValid) {
      if (typeof endDate2 === "object") {
        if (import_luxon.DateTime.isDateTime(endDate2) && endDate2.isValid) {
          this.endDate = endDate2;
        } else if (endDate2 instanceof Date) {
          this.endDate = import_luxon.DateTime.fromJSDate(endDate2);
        } else {
          throw RangeError(`The 'endDate' must be a luxon.DateTime or Date or string`);
        }
      } else if (typeof endDate2 === "string") {
        const format = typeof this.locale.format === "string" ? this.locale.format : import_luxon.DateTime.parseFormatForOpts(this.locale.format);
        if (import_luxon.DateTime.fromISO(endDate2).isValid) {
          this.endDate = import_luxon.DateTime.fromISO(endDate2);
        } else if (import_luxon.DateTime.fromFormat(endDate2, format, { locale: import_luxon.DateTime.now().locale }).isValid) {
          this.endDate = import_luxon.DateTime.fromFormat(endDate2, format, { locale: import_luxon.DateTime.now().locale });
        } else {
          const invalid = import_luxon.DateTime.fromFormat(endDate2, format, { locale: import_luxon.DateTime.now().locale }).invalidExplanation;
  ...
  */
  ...
  formatDate(date, format = this.locale.format) {
    if (typeof format === "object") {
      ...
      if (import_luxon.Settings.defaultLocale === null) {
        const locale = import_luxon.DateTime.now().locale;
  ...
  */
  validateInput(range) {
    ...
    if (this.timePicker) {
      ...
      startDate = import_luxon.DateTime.fromSeconds(secs * Math.round(startDate.toSeconds() / secs));
    } else {
    ...
    const shiftStep = this.timePicker ? this.timePickerStepSize.as("seconds") : import_luxon.Duration.fromObject({ days: 1 }).as("seconds");
    ...
    if (this.timePicker) {
      const secs = this.timePickerStepSize.as("seconds");
      endDate2 = import_luxon.DateTime.fromSeconds(secs * Math.round(endDate2.toSeconds() / secs));
    } else {
  ...
  */
  renderCalendar(side) {
    ...
    for (let [index, dayOfWeek] of this.locale.daysOfWeek.entries()) {
      html += "<th";
      if (this.weekendDayClasses && this.weekendDayClasses.length && import_luxon.Info.getWeekendWeekdays().includes(index + 1))
    ...
    for (let row = 0; row < 6; row++) {
      ...
      for (let col = 0; col < 7; col++) {
        var classes = [];
        if (this.todayClasses && this.todayClasses.length && calendar[row][col].hasSame(import_luxon.DateTime.now(), "day"))
          classes.push(this.todayClasses);
        if (this.weekendClasses && this.weekendClasses.length && import_luxon.Info.getWeekendWeekdays().includes(calendar[row][col].weekday))
  ...
  renderTimePicker(side) {
    ...
    for (var i = start; i <= start + 23; i += this.timePickerOpts.hourStep) {
      ...
      if (this.timePicker24Hour) {
        ...
        const i_12 = import_luxon.DateTime.fromFormat(`${i % 24}`, "H").toFormat("h");
        const i_ampm = import_luxon.DateTime.fromFormat(`${i % 24}`, "H").toFormat("a", { locale: "en-US" });
        if (ampm == i_ampm) {
          if (!disabled && i == selected.hour) {
    ...
    if (!this.timePicker24Hour) {
      ...
      if (selected.toFormat("a", { locale: "en-US" }) === "AM")
        html += " selected";
      html += `>${import_luxon.Info.meridiems()[0]}</option><option value="PM"${pm_html}`;
      if (selected.toFormat("a", { locale: "en-US" }) === "PM")
        html += " selected";
      html += `>${import_luxon.Info.meridiems()[1]}</option>`;
      html += "</select>";
      if (this.externalStyle === "bulma")
        html += "</div>";
    }
  ...
  */
  move() {
    ...
    var parentRightEdge = (0, import_jquery.$)(window).width();
    ...
    if (this.opens === "left") {
      ...
      if (containerWidth + containerRight > (0, import_jquery.$)(window).width()) {
        this.container.css({
          top: containerTop,
          right: "auto",
          left: 9
      ...
      } else if (containerLeft + containerWidth > (0, import_jquery.$)(window).width()) {
        this.container.css({
          top: containerTop,
          left: "auto",
          right: 0
      ...
      if (containerLeft + containerWidth > (0, import_jquery.$)(window).width()) {
        this.container.css({
          top: containerTop,
          left: "auto",
          right: 0
  ...
  */
  show(e) {
    ...
    (0, import_jquery.$)(document).on("mousedown.daterangepicker", this._outsideClickProxy).on("touchend.daterangepicker", this._outsideClickProxy).on("click.daterangepicker", "[data-toggle=dropdown]", this._outsideClickProxy).on("focusin.daterangepicker", this._outsideClickProxy);
    (0, import_jquery.$)(window).on("resize.daterangepicker", function(e2) {
      this.move(e2);
    }.bind(this));
  ...
  */
  hide(e) {
    ...
    if (this.element.triggerHandler("beforeHide.daterangepicker", this))
      return;
    (0, import_jquery.$)(document).off(".daterangepicker");
    (0, import_jquery.$)(window).off(".daterangepicker");
    this.container.hide();
  ...
  */
  outsideClick(e) {
    var target = (0, import_jquery.$)(e.target);
    if (
      // ie modal dialog fix
  ...
  */
  clickPrev(e) {
    var cal = (0, import_jquery.$)(e.target).parents(".drp-calendar");
  ...
  */
  hoverDate(e) {
    if (!(0, import_jquery.$)(e.target).hasClass("available")) return;
    let title = (0, import_jquery.$)(e.target).attr("data-title");
    const row = title.substring(1, 2);
    const col = title.substring(3, 4);
    const cal = (0, import_jquery.$)(e.target).parents(".drp-calendar");
  ...
  */
  hoverRange(e) {
    ...
    this.container.find(".drp-calendar tbody td").each(function(index, el) {
      if ((0, import_jquery.$)(el).hasClass("week")) return;
      const title = (0, import_jquery.$)(el).attr("data-title");
      const row = title.substring(1, 2);
      const col = title.substring(3, 4);
      const cal = (0, import_jquery.$)(el).parents(".drp-calendar");
      const dt = cal.hasClass("left") ? leftCalendar.calendar[row][col] : rightCalendar.calendar[row][col];
      let classAdded = false;
      if (dt.hasSame(dates[0], "day"))
        classAdded = (0, import_jquery.$)(el).addClass("start-hover").length > 0;
      if (dt.hasSame(previousDates[0], "day"))
        classAdded = (0, import_jquery.$)(el).addClass("start-date").length > 0;
      if (dt.hasSame(dates[1], "day"))
        classAdded = (0, import_jquery.$)(el).addClass("end-hover").length > 0;
      if (previousDates[1] != null && dt.hasSame(previousDates[1], "day"))
        classAdded = (0, import_jquery.$)(el).addClass("end-date").length > 0;
      if (dt.startOf("day") >= dates[0].startOf("day") && dt.startOf("day") <= dates[1].startOf("day"))
        classAdded = (0, import_jquery.$)(el).addClass("range-hover").length > 0;
      if (dt.startOf("day") >= previousDates[0].startOf("day") && previousDates[1] != null && dt.startOf("day") <= previousDates[1].startOf("day"))
        classAdded = (0, import_jquery.$)(el).addClass("in-range").length > 0;
      if (!classAdded) {
        (0, import_jquery.$)(el).removeClass("start-hover");
        (0, import_jquery.$)(el).removeClass("end-hover");
        (0, import_jquery.$)(el).removeClass("start-date");
        (0, import_jquery.$)(el).removeClass("end-date");
        (0, import_jquery.$)(el).removeClass("in-range");
        (0, import_jquery.$)(el).removeClass("range-hover");
      }
    });
  }
  /**
  ...
  */
  clickDate(e) {
    if (!(0, import_jquery.$)(e.target).hasClass("available")) return;
    var title = (0, import_jquery.$)(e.target).attr("data-title");
  ...
  */
  monthOrYearChanged(e) {
    var isLeft = (0, import_jquery.$)(e.target).closest(".drp-calendar").hasClass("left"), leftOrRight = isLeft ? "left" : "right", cal = this.container.find(".drp-calendar." + leftOrRight);
  ...
  */
  timeChanged(e) {
    const time = (0, import_jquery.$)(e.target).closest(".calendar-time");
    ;
    ...
    if (!this.timePicker24Hour) {
      ...
      if (ampm != import_luxon.DateTime.fromFormat(`${hour}`, "H").toFormat("a", { locale: "en-US" })) {
        time.find(".hourselect > option").each(function() {
          const hidden = (0, import_jquery.$)(this).attr("hidden") || false;
          (0, import_jquery.$)(this).attr("hidden", hidden);
        });
        const h = import_luxon.DateTime.fromFormat(`${hour}`, "H").toFormat("h");
        hour = import_luxon.DateTime.fromFormat(`${h}${ampm}`, "ha", { locale: "en-US" }).hour;
      }
    }
    var minute = 0;
    if (this.timePickerOpts.showMinutes) {
  ...
  */
  elementChanged(e) {
    ...
    if (dateString.length === 2) {
      start = import_luxon.DateTime.fromFormat(dateString[0], format, { locale: import_luxon.DateTime.now().locale });
      end = import_luxon.DateTime.fromFormat(dateString[1], format, { locale: import_luxon.DateTime.now().locale });
    }
    if (this.singleDatePicker || start === null || end === null) {
      start = import_luxon.DateTime.fromFormat(this.element.val(), format, { locale: import_luxon.DateTime.now().locale });
      end = start;
    }
    if (!start.isValid || !end.isValid) {
      return;
  ...
  updateElement() {
    ...
    if (this.altInput != null) {
      if (this.altFormat == null) {
        ...
        if (this.singleDatePicker) {
          if ((0, import_jquery.$)(this.altInput).is("input"))
            (0, import_jquery.$)(this.altInput).val(this.startDate.toISO({ format: "basic", precision, includeOffset: false }));
        } else {
          if (this.altInput.every((x) => (0, import_jquery.$)(x).is("input"))) {
            (0, import_jquery.$)(this.altInput[0]).val(this.startDate.toISO({ format: "basic", precision, includeOffset: false }));
            if (this.endDate) {
              (0, import_jquery.$)(this.altInput[1]).val(this.endDate.toISO({ format: "basic", precision, includeOffset: false }));
            } else {
              (0, import_jquery.$)(this.altInput[1]).val(null);
            }
          }
        }
      } else {
        if (this.singleDatePicker) {
          if ((0, import_jquery.$)(this.altInput).is("input"))
            (0, import_jquery.$)(this.altInput).val(typeof this.altFormat === "function" ? this.altFormat(this.startDate) : this.formatDate(this.startDate, this.altFormat));
        } else {
          if (this.altInput.every((x) => (0, import_jquery.$)(x).is("input"))) {
            (0, import_jquery.$)(this.altInput[0]).val(typeof this.altFormat === "function" ? this.altFormat(this.startDate) : this.formatDate(this.startDate, this.altFormat));
            if (this.endDate) {
              (0, import_jquery.$)(this.altInput[1]).val(typeof this.altFormat === "function" ? this.altFormat(this.endDate) : this.formatDate(this.endDate, this.altFormat));
            } else {
              (0, import_jquery.$)(this.altInput[1]).val(null);
            }
          }
...
if (!import_jquery.$.fn.daterangepicker) {
  import_jquery.$.fn.daterangepicker = function(options, callback) {
    const implementOptions = import_jquery.$.extend(true, {}, import_jquery.$.fn.daterangepicker.defaultOptions, options);
    this.each(function() {
      const el = (0, import_jquery.$)(this);
