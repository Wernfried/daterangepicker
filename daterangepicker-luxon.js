...
(function (root, factory) {
    ...
    var DateRangePicker = function (element, options, cb) {
        ...
        if (typeof options.startDate === 'undefined' && typeof options.endDate === 'undefined' && typeof this.locale.format === 'string') {
            if ($(this.element).is(':text')) {
                ...
                if (split.length == 2) {
                    start = DateTime.fromFormat(split[0], this.locale.format);
                    end = DateTime.fromFormat(split[1], this.locale.format);
                } else if (this.singleDatePicker && val !== "") {
                    start = DateTime.fromFormat(val, this.locale.format);
                    end = DateTime.fromFormat(val, this.locale.format);
                }
        ...
        if (!this.singleDatePicker && Array.isArray(options.ranges)) {
            for (let range in options.ranges) {
                ...
                if (['string', 'object'].includes(typeof options.ranges[range][0])) {
                    if (options.ranges[range][0] instanceof DateTime) {
                        start = options.ranges[range][0];
                    } else if (DateTime.fromISO(options.ranges[range][0]).isValid) {
                        start = DateTime.fromISO(options.ranges[range][0]);
                    } else {
                        console.error(`Option 'ranges.${range}' is not a valid string`);
                    }
                }
                if (['string', 'object'].includes(typeof options.ranges[range][1])) {
                    if (options.ranges[range][1] instanceof DateTime) {
                        end = options.ranges[range][1];
                    } else if (DateTime.fromISO(options.ranges[range][1]).isValid) {
    ...
    DateRangePicker.prototype = {
        ...
        setStartDate: function (startDate) {
            if (typeof startDate === 'string')
                this.startDate = DateTime(startDate, this.locale.format);
...
setEndDate: function (endDate) {
    if (typeof endDate === 'string')
        this.endDate = DateTime(endDate, this.locale.format);
...
updateMonthsInView: function () {
    if (this.endDate) {
        ...
        if (!this.linkedCalendars && (this.endDate.month != this.startDate.month || this.endDate.year != this.startDate.year)) {
            ...
            this.rightCalendar.month = this.startDate.date(2).plus({ 1, 'month');
        }
    ...
    if (this.maxDate && this.linkedCalendars && !this.singleDatePicker && this.rightCalendar.month > this.maxDate) {
        ...
        this.leftCalendar.month = this.maxDate.date(2).minus({ 1, 'month');
    }
},
...
renderCalendar: function (side) {
    ...
    var daysInMonth = DateTime([year, month]).daysInMonth();
    var firstDay = DateTime([year, month, 1]);
    var lastDay = DateTime([year, month, daysInMonth]);
    var lastMonth = DateTime(firstDay).minus({ 1, 'month').month;
    var lastYear = DateTime(firstDay).minus({ 1, 'month').year;
    var daysInLastMonth = DateTime([lastYear, lastMonth]).daysInMonth();
    ...
    var curDate = DateTime([lastYear, lastMonth, startDay, 12, minute, second]);

    var col, row;
    for (var i = 0, col = 0, row = 0; i < 42; i++, col++, curDate = DateTime(curDate).plus({ 24, 'hour')) {
        if (i > 0 && col % 7 === 0) {
            col = 0;
            row++;
        }
        calendar[row][col] = curDate.hour(hour).minute(minute).second(second);
        curDate.hour(12);
    ...
    for (var row = 0; row < 6; row++) {
        ...
        for (var col = 0; col < 7; col++) {
            ...
            if (this.minDate && calendar[row][col] < this.minDate, 'day'))
...
renderTimePicker: function (side) {
    ...
    for (var i = start; i <= end; i += this.timePickerStepHour) {
        ...
        var time = selected.hour(i_in_24);
        var disabled = false;
        if (minDate && time.minute(59) < minDate))
        disabled = true;
    ...
    for (var i = 0; i < 60; i += this.timePickerStepMinute) {
        ...
        var time = selected.minute(i);

        var disabled = false;
        if (minDate && time.second(59) < minDate))
        disabled = true;
...
updateFormInputs: function () {
    ...
    if (this.singleDatePicker || (this.endDate && (this.startDate < this.endDate) || this.startDate.isSame(this.endDate)))) {
...
clickPrev: function (e) {
    ...
    if (cal.hasClass('left')) {
        this.leftCalendar.month.minus({ 1, 'month');
        if (this.linkedCalendars)
            this.rightCalendar.month.minus({ 1, 'month');
    } else {
        this.rightCalendar.month.minus({ 1, 'month');
    }
    this.updateCalendars();
...
clickNext: function (e) {
    var cal = $(e.target).parents('.drp-calendar');
    if (cal.hasClass('left')) {
        this.leftCalendar.month.plus({ 1, 'month');
    } else {
        this.rightCalendar.month.plus({ 1, 'month');
        if (this.linkedCalendars)
            this.leftCalendar.month.plus({ 1, 'month');
    }
    this.updateCalendars();
...
calculateChosenLabel: function () {
    ...
    for (var range in this.ranges) {
        if (this.timePicker) {
            ...
            if (this.startDate.toFormat(format) == this.ranges[range][0].toFormat(format) && this.endDate.toFormat(format) == this.ranges[range][1].toFormat(format)) {
                customRange = false;
...
monthOrYearChanged: function (e) {
    ...
    if (isLeft) {
        ...
        if (this.linkedCalendars)
            this.rightCalendar.month = this.leftCalendar.month.plus({ 1, 'month');
    } else {
        this.rightCalendar.month.month(month).year(year);
        if (this.linkedCalendars)
            this.leftCalendar.month = this.rightCalendar.month.minus({ 1, 'month');
    }
    this.updateCalendars();
},
...
elementChanged: function () {
    ...
    if (dateString.length === 2) {
        start = DateTime(dateString[0], this.locale.format);
        end = DateTime(dateString[1], this.locale.format);
    }

    if (this.singleDatePicker || start === null || end === null) {
        start = DateTime(this.element.val(), this.locale.format);
        end = start;
    }
